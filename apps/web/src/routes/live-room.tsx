...
export function LiveRoomPage() {
  ...
  return frame(<div className="live-room-shell">{!effectiveGrant?.is_host && <header className="live-topbar"><Brand /></header>}<div>
    {error ? <div className="live-prejoin"><ErrorState title="Не удалось открыть комнату" description={error instanceof Error && error.message === 'INVITATION_REQUIRED' ? 'Откройте ссылку-приглашение от организатора или войдите в рабочее пространство.' : liveError(error)} onRetry={() => { void entry.refetch(); if (joined) void room.refetch() }}><Button variant="default" component={Link} to="/login">Войти в рабочее пространство</Button></ErrorState></div>
      : entry.isPending || (joined && room.isPending) ? <LiveConnecting title={joined ? 'Открываем комнату' : 'Готовим встречу'} description="Загружаем пространство разговора" />
      : entry.data ? <RoomPrejoin room={entry.data.room} grant={effectiveGrant} invite={invite} onJoin={join} /> : null}
    <div className="live-prejoin-footer"><Anchor component={Link} to="/live">К голосовым встречам</Anchor></div>
  </div></div>)
}
