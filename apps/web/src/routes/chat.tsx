...
type Turn = { question: string; result: AssistantResult }
...
export function ChatPage() {
  ...
  const ask = useMutation({
    mutationFn: (value: string) => {
      ...
      const context: ConversationMessage[] = turns.slice(-10).flatMap((turn) => [
        ...
        { role: 'assistant' as const, content: (turn.result.answer || 'В источниках не удалось найти подтверждение.').slice(0, 6000) },
      ])
      while (context.reduce((sum, message) => sum + message.content.length, 0) > 40_000) context.splice(0, 2)
      return talkToAssistant(value, context, controller.signal)
    },
  ...
  const ready = !config.isError && Boolean(config.data)
  function submit() {
  ...
  return <div className={`page workspace-chat ${turns.length || ask.isPending ? 'has-conversation' : ''}`}>
    <header className="page-header workspace-chat-header">
      <div><PageHeading title="Чат" /><p className="workspace-chat-subtitle">Ваш ассистент</p></div>
    ...
    {turns.length === 0 && !ask.isPending ? <section className="workspace-chat-welcome" aria-labelledby="chat-welcome">
      ...
      {ask.isPending && <article className="workspace-chat-turn"><h2 className="workspace-chat-question">{ask.variables}</h2><div className="workspace-chat-thinking" role="status"><Loader size="sm" />Готовлю ответ…</div></article>}
      <div ref={bottom} />
    </section>}
    <div className="workspace-chat-composer-wrap">
      ...
      <p className="workspace-chat-note">{config.isPending ? 'Подключаем ассистента…' : 'Общение, помощь с Soyle и поиск по встречам.'}</p>
    </div>
  </div>
}
