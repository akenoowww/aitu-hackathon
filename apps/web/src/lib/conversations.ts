...
const pendingResultSchema = z.object({ mode: z.enum(['pending', 'failed']), answer: z.literal(''), activity: z.enum(['thinking', 'creating']) })
...
export const conversationsApi = {
  ...
  startTurn: (id: string, turn: { id: string; question: string; state: 'pending' | 'failed'; activity: 'thinking' | 'creating' }, signal?: AbortSignal) => request(`/${encodeURIComponent(id)}/turns/pending`, turnSchema, turn, signal),
