...
export function MeetingChat({ meetingId }: { meetingId: string }) {
  ...
  return <section className="transcript-panel rag-chat" aria-labelledby="meeting-chat-title">
    <Title order={2} size="h3" id="meeting-chat-title">Вопросы к встрече</Title>
    <Text component="p" c="dimmed">Задайте вопрос о решениях, участниках или договорённостях. К ответу будут приложены цитаты из стенограммы. Каждый вопрос рассматривается отдельно.</Text>
    {config && <Text component="p" size="sm" c="dimmed">{cloud
      ? 'В этом режиме вопросы и фрагменты встречи передаются в OpenAI. При подготовке поиска может передаваться вся стенограмма.'
      : 'Для ответов и поиска используются модели, настроенные на локальном сервере.'}</Text>}
    ...
    {config && !ready && !status.isError && <Flex direction="column" gap="md" align="start">
      {status.data?.status === 'failed' && <Text component="p" c="red">Не удалось подготовить встречу. Проверьте подключение к модели и повторите попытку.</Text>}
      <Button size="md" onClick={() => index.mutate()} disabled={busy || !status.data || configuration.isError} loading={busy}>
        {busy ? 'Подготавливаем встречу…' : status.data?.status === 'failed' ? 'Повторить подготовку' : 'Подготовить встречу для вопросов'}
      </Button>
    </Flex>}
    ...
    <div aria-live="polite" aria-relevant="additions" className="rag-turns">
      {turns.map((turn, number) => <article key={number} className="rag-turn">
        ...
        {turn.result.status === 'insufficient_evidence'
          ? <Text component="p">В найденных фрагментах недостаточно данных для ответа. Уточните вопрос или проверьте стенограмму.</Text>
          : turn.result.claims.map((claim, i) => <div key={i}>
            <Text component="p">{claim.text}</Text>
            {claim.citations.map((citation, j) => <Disclosure key={j} className="rag-source" label={`Цитата из стенограммы ${i + 1}.${j + 1}`}>
    ...
    <form onSubmit={(event) => { event.preventDefault(); if (ready && question.trim() && !ask.isPending) ask.mutate(question.trim()) }}>
      <Textarea label="Ваш вопрос" id="meeting-question" size="md" value={question} onChange={(event) => setQuestion(event.target.value)}
        placeholder="Например: какой бюджет согласовали?" maxLength={2000} disabled={!ready || ask.isPending} rows={3} />
      <Button type="submit" size="md" disabled={!ready || !question.trim() || !config || configuration.isError} loading={ask.isPending}>
        {ask.isPending ? 'Ищем ответ…' : 'Задать вопрос'}
      </Button>
    </form>
