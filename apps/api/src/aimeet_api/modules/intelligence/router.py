...
def read_board(meeting_id: uuid.UUID, user: CurrentUser, db: DatabaseDep):
    owned(db, user, meeting_id)
    board = db.get(MeetingBoard, meeting_id)
    return output(board) if board else BoardOutput(
        version=0, status='idle', error_code=None, summary=[], cards=[], progress=0,
    )


@router.post('/generate', response_model=BoardOutput, dependencies=[Depends(require_csrf)],
...
def generate(meeting_id: uuid.UUID, user: CurrentUser, db: DatabaseDep):
    ...
    if not meeting.transcript.strip() or (meeting.source_type == 'audio'
                                         and meeting.status != 'transcribed'):
    ...
    db.execute(update(MeetingBoard).execution_options(synchronize_session=False).where(
        MeetingBoard.meeting_id == meeting_id, MeetingBoard.status.in_(['idle', 'failed']),
    ).values(status='queued', attempts=0, progress=0, error_code=None,
             source_hash=digest(meeting.transcript), lease_until=None, lease_token=None))
    db.commit()
    db.refresh(board)
    return output(board)


@router.post('/cards', response_model=BoardOutput, status_code=201,
             dependencies=[Depends(require_csrf)], operation_id='createMeetingCard')
def create_card(meeting_id: uuid.UUID, payload: CardCreate, user: CurrentUser, db: DatabaseDep):
    ...
    return save_cards(db, board, payload.version, [*board.cards, card])


@router.post('/cards/{card_id}', response_model=BoardOutput,
             dependencies=[Depends(require_csrf)], operation_id='updateMeetingCard')
def edit_card(meeting_id: uuid.UUID, card_id: uuid.UUID, payload: CardUpdate,
    ...
    return save_cards(db, board, payload.version,
                      [card if c['id'] == str(card_id) else c for c in board.cards])


KIND = {'task': 'Поручение', 'decision': 'Решение', 'topic': 'Тема',
        'question': 'Открытый вопрос', 'risk': 'Риск'}
STATUS = {'todo': 'К выполнению', 'doing': 'В работе', 'blocked': 'Заблокировано',
          'done': 'Готово', 'dismissed': 'В архиве'}
...
def export_board(meeting_id: uuid.UUID, format: Literal['json', 'csv', 'ics'],
                 user: CurrentUser, db: DatabaseDep):
    meeting = owned(db, user, meeting_id)
    board = db.get(MeetingBoard, meeting_id)
    if board is None:
        raise RagError('BOARD_EMPTY', 409)
    data = output(board).model_dump(mode='json')
    if format == 'json':
        body = json.dumps({'format_version': 1, 'meeting_title': meeting.title, **data},
                          ensure_ascii=False, indent=2)
        media = 'application/json'
    elif format == 'csv':
        buffer = io.StringIO(newline='')
        writer = csv.writer(buffer)
        writer.writerow(['Тип', 'Суть', 'Описание', 'Ответственный', 'Дата', 'Срок как озвучен',
                         'Приоритет', 'Статус', 'Проверено пользователем', 'Источник', 'Цитата'])
        for sentence in data['summary']:
            writer.writerow([safe_cell(c) for c in ['Выжимка', sentence['text'], '', '', '', '',
                                                    '', '', '', 'ИИ', sentence['quote']]])
        for card in data['cards']:
            writer.writerow([safe_cell(c) for c in [KIND[card['kind']], card['title'],
                card['description'], card['assignee'], card['due_date'], card['due_text'],
                PRIORITY[card['priority']], STATUS[card['status']],
                'Да' if card['reviewed'] else 'Нет',
                'ИИ' if card['origin'] == 'ai' else 'Вручную', card['quote']]])
        body, media = '\ufeff' + buffer.getvalue(), 'text/csv'
    else:
        lines = ['BEGIN:VCALENDAR', 'VERSION:2.0', 'PRODID:-//Aimeet//Meeting tasks//RU',
                 'CALSCALE:GREGORIAN']
        for card in data['cards']:
            if (card['kind'] != 'task' or not card['due_date'] or not card['reviewed']
                    or card['status'] in {'done', 'dismissed'}):
                continue
            from datetime import date
            due = date.fromisoformat(card['due_date'])
            lines += ['BEGIN:VEVENT', f"UID:{card['id']}@aimeet.local",
