...
Статус: первый слой, 11 сентября 2026. Основной сценарий из ТЗ — локальный исходник встречи → структурированный протокол → проверенные поручения и экспорт. Архитектура рассчитана на развитие этого сценария; первый слой реализует только доступ и надёжное хранение текстовых исходников.
...
flowchart LR
  ...
  API --> DB[(PostgreSQL)]
  API -. слой 2: долговечные задания .-> Jobs[(Задания / leases / retries)]
  Jobs -.-> Worker[Локальный worker]
  Worker -.-> STT[Faster-Whisper]
  Worker -.-> LLM[Ollama]
  Worker -.-> Files[(Аудио / версии артефактов)]
  Capture[Vexa: необязательный захват] -. будущий адаптер .-> API
```

Сплошные связи реализуются в слое 1. Пунктир — архитектурные границы будущих слоёв, а не запущенные сервисы.
...
Версии зависимостей фиксируются lock-файлами. Не добавляем Redux для серверного кеша, Next.js ради статической панели, Redis без задачи очереди, MinIO без нужды в объектном хранилище или Kubernetes без эксплуатационного основания.
...
1. **Основа:** доступ, текстовые встречи, миграции, сборка, контейнеры и проверки. Текущая точка остановки.
2. **Локальный конвейер:** файлы MP3/WAV/M4A, безопасное декодирование, Faster-Whisper, очередь с восстановлением, ресурсы CPU/GPU, отмена и progress.
...
Диаризация, чат по встрече и Vexa — отдельные расширения после основного пути. Следующий слой не начинается в рамках текущего запроса.
