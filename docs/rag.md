...
перезагрузке страницы или выходе. Каждый вопрос выполняет самостоятельный поиск;
предыдущие реплики не передаются модели. Нагрузка общего точного поиска растёт с
объёмом архива: стенограммы проверяются на актуальность, узлы загружаются для BM25.
